...
public class DatabaseMetaUtil {
    ...
    private final static String DATABASE_NAME = "zero";
    ...
    private void getDatabaseMetaData() {
        try {
            if (dbMetaData == null) {
                ...
                String url = "jdbc:mysql://192.168.200.118:3306?characterEncoding=utf8&useSSL=false&serverTimezone=GMT%2B8&nullNamePatternMatchesAll=true";
                String user = "root";
     ...
     * 获得一个表的主键信息
     */
    public String getPrimaryKeyBySchemaNameAndTableName() {
        String primaryKey = null;
        try {
            ResultSet rs = dbMetaData.getPrimaryKeys("zero", null, "t_table_info");
            while (rs.next()) {
    ...
    public void getTableColumns() {
        ...
        try {
            ...
            ResultSet rs = dbMetaData.getColumns("zero", null, "t_table_info", "%");
            while (rs.next()) {
                ...
                System.out.println("表的架构:" + tableSchemaName);
                System.out.println("表名:" + tableName_);
                System.out.println("列名:" + columnName);
                System.out.println("对应的java.sql.Types类型:" + dataType);
                System.out.println("java.sql.Types类型:" + dataTypeName);
                System.out.println("列大小:" + columnSize);
                System.out.println("小数位数:" + decimalDigits);
                System.out.println("基数（通常是10或2）:" + numPrecRadix);
                System.out.println("是否允许为null:" + nullAble);
                System.out.println("列描述:" + remarks);
                System.out.println("默认值:" + columnDef);
                System.out.println("sql数据类型:" + sqlDataType);
                System.out.println("SQL日期时间分:" + sqlDatetimeSub);
                System.out.println("char类型的列中的最大字节数:" + charOctetLength);
                System.out.println("表中列的索引（从1开始）:" + ordinalPosition);
    ...
    public void getAllPrimaryKeys(String schemaName, String tableName) {
        try {
            ResultSet rs = dbMetaData.getPrimaryKeys("zero", "root", "t_table_info");
            while (rs.next()) {
                String columnName = rs.getString("COLUMN_NAME");// 列名
...
//        metaData.getTableColumns();
        System.out.println(metaData.getPrimaryKeyBySchemaNameAndTableName());
    }
