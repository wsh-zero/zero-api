...
public class GenerateUtil {
    ...
    public static void main(String[] args) throws IOException {
        File file = new File("D:\\temp\\fieldmeta\\res\\template");
        if(!file.exists()) {
            file.mkdirs();
...
//        g.generateByTables(startWithI,projectName,packageName,tableName);
    ...
    private DataSourceConfig getDataSourceConfig() {
        return new DataSourceConfig().setDbType(DbType.MYSQL)
                .setUrl("jdbc:mysql://192.168.200.118:3306/zero?serverTimezone=Hongkong&nullCatalogMeansCurrent=true&characterEncoding=UTF-8&useSSL=false")
                .setUsername("root")
                .setPassword("123456")
